...
router.post("/api/inventory",
    ...
    async (req, res, next) => {
        const id = await dbi.createRecord(req.body.description, req.body.quantity, req.body.price);
...
router.delete("/api/inventory/:id", async (req, res, next) => {
    console.log("DELETE");
    const affectedRows = await dbi.deleteRecord(req.params.id);
    res.write(JSON.stringify({ 'affectedRows': affectedRows }));
    res.end();
});
